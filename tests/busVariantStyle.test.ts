...
describe("bus variant styling", () => {
  ...
  test("selected variant style wins over active/inactive opacity", () => {
    expect(variantStyleForShape("shape-a", "shape-a", new Set(), "#abc")).toEqual({
      color: "#abc",
      weight: 5,
      opacity: 0.95,
      bringToFront: true,
  ...
  test("active unselected variants are faint and inactive variants are hidden", () => {
    expect(variantStyleForShape("shape-a", null, new Set(["shape-a"]), "#abc")).toEqual({
      color: "#abc",
      weight: 3,
      opacity: 0.35,
      bringToFront: false,
    });
    expect(variantStyleForShape("shape-b", null, new Set(["shape-a"]), "#abc")).toEqual({
      color: "#abc",
      weight: 3,
      opacity: 0,
      bringToFront: false,
