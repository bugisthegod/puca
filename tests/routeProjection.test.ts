...
if (!built) throw new Error("fixture setup: expected buildRouteLine to succeed");
...
describe("projectOntoRoute", () => {
  test("reports offRoute when the vehicle is far from the line", () => {
    ...
    const r = projectOntoRoute(54.0, -5.95, routeLine, routeLengthMeters, null, null, 1000);
    expect(r.offRoute).toBe(true);
  });

  test("projects an on-route vehicle to a bounded distance and defaults speed to 0", () => {
    // Midpoint of the line.
    const r = projectOntoRoute(53.0, -5.95, routeLine, routeLengthMeters, null, null, 1000);
  ...
  test("uses defaultSpeedMps when there is no previous ping to delta against", () => {
    const r = projectOntoRoute(53.0, -5.95, routeLine, routeLengthMeters, null, null, 1000, 150, 25, 15);
    expect(r.offRoute).toBe(false);
    if (r.offRoute) return;
  ...
  test("clamps computed speed to maxSpeedMps", () => {
    ...
    const r = projectOntoRoute(53.0, -5.95, routeLine, routeLengthMeters, 0, 1000, 2000, 150, 25, 0);
    expect(r.offRoute).toBe(false);
    if (r.offRoute) return;
  ...
  test("falls back to defaultSpeedMps when deltaDist is non-positive (no forward motion)", () => {
    ...
    const first = projectOntoRoute(53.0, -5.95, routeLine, routeLengthMeters, null, null, 1000, 150, 25, 0);
    if (first.offRoute) throw new Error("fixture: expected midpoint to be on-route");
    const stationary = projectOntoRoute(
      53.0, -5.95, routeLine, routeLengthMeters,
      first.targetDistanceAlongRoute, 1000, 2000,
      150, 25, 15,
    );
  ...
  test("honors a custom offRouteMeters threshold", () => {
    ...
    const tight = projectOntoRoute(vehicleLat, -5.95, routeLine, routeLengthMeters, null, null, 1000, 150);
    const loose = projectOntoRoute(vehicleLat, -5.95, routeLine, routeLengthMeters, null, null, 1000, 500);
  ...
  test("respects the bounded [0, routeLengthMeters] clamp for returned distance", () => {
    ...
    const r = projectOntoRoute(53.0, -5.9, routeLine, routeLengthMeters, null, null, 1000);
    expect(r.offRoute).toBe(false);
    if (r.offRoute) return;
