...
When arrivals suddenly disappear or buses go to zero while production logs still show vehicles, diagnose GTFS feed drift before regenerating. Compare the live `feed_info.txt` with the local `gtfs/feed_info.txt`; sometimes the lightweight feed metadata changes before the full zip is republished.
