...
import {
	...
	buildBusPopupHTML as buildBusPopupContentHTML,
} from "./busPopup";
import {
	busRouteColor,
...
interface UseBusMarkersOptions {
	...
	getAnimationDurationMs: BusAnimationDurationStrategy;
...
export function useBusMarkers({
	...
	function applyVariantStyles() {
		...
		for (const [shapeId, polylines] of Object.entries(
	...
	function buildBusPopupHTML(
		...
		return buildBusPopupContentHTML(bus, trip, {
			showRouteJump: Boolean(
				onSelectBusRouteRef.current &&
					currentBusRouteRef.current !== bus.routeShortName,
			),
		});
	}
	...
	function scrollPopupToCurrent(marker: L.Marker) {
		requestAnimationFrame(() => {
			...
			const currentOffsetInWrap =
				currentRect.top - wrapRect.top + wrap.scrollTop;
	...
	async function loadBusTrip(bus: BusVehicle, marker: L.Marker) {
		...
		if (
			...
			existingContent.includes("popup-table-wrap")
		) {
	...
	useEffect(() => {
		...
		const filteredBuses =
			currentBusRoute && busDirection
				? buses.filter(
						(b) =>
							b.routeShortName === currentBusRoute &&
							String(b.directionId) === busDirection,
					)
				: buses;
		...
		for (const bus of filteredBuses) {
			...
			if (existing) {
				...
				const prevDistance: number | null = null;
				let currentDistance: number | null = null;
				const animStartPerfMs: number | null = null;
				let offRoute = true;
		...
		selectedShapeIdRef.current = reconcileSelectedVariant(
