...
export function buildTrainStatusText(
	status: string,
	late: number | null,
): string {
	...
	if (late < 0) {
		...
		return n === 1
			? t("popup.status.early.one")
			: t("popup.status.early.many", { n });
	}
	return late === 1
		? t("popup.status.late.one")
...
export function buildTrainPopupErrorHTML(train: Train): string {
	return buildTrainPopupShell(
		train,
		`<div class="popup-message">${t("popup.train.error")}</div>`,
	);
}

export function buildTrainPopupWithMovements(
	train: Train,
	movements: TrainMovement[],
): string {
	...
	const deriveStopType = (
		...
		const isStaleNextReplaced =
			hasProgressNextMatch && movement.stopType === "N";
	...
	const rows = movements
		.map((m, index) => {
			...
			const showArr =
				actArr !== "—" ? actArr : expArr !== "—" ? expArr : schArr;
			const showDep =
				actDep !== "—" ? actDep : expDep !== "—" ? expDep : schDep;
