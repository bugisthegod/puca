...
export function buildBusIconSpec(bus: BusVehicle, operator: BusOperator): BusIconSpec {
  const classes = ["bus-marker", busOperatorMarkerClass(operator), bus.stale ? "bus-marker--stale" : ""]
    .filter(Boolean).join(" ");
