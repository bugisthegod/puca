...
function pickProgressIndexes(
	...
	const currentIndex = progress?.currentStation
		? movements.findIndex(
				(m) =>
					normalizeStationName(m.stationName) ===
					normalizeStationName(progress.currentStation),
			)
	...
	const nextIndex = progress?.nextStation
		? movements.findIndex(
				(m) =>
					normalizeStationName(m.stationName) ===
					normalizeStationName(progress.nextStation!),
			)
...
export function useTrainMarkers({
	...
	const onMarkerClickRef = useRef<
		(trainCode: string, options?: { preserveSummary?: boolean }) => void
	>(() => {});
	...
	async function buildTrainFocusSummary(
		entry: TrainMarkerEntry,
		targetStation: Station,
	): Promise<TrainFocusSummary> {
		let movements: TrainMovement[] = [];
		try {
			const res = await fetch(
				`/api/train/${encodeURIComponent(entry.train.code)}`,
			);
		...
		const targetMovement = movements.find(
			(m) => m.stationCode === targetStation.code,
		);
		const targetIndex = targetMovement ? movements.indexOf(targetMovement) : -1;
		const { currentIndex, nextIndex } = pickProgressIndexes(
			entry.train,
			movements,
		);
	...
	async function onMarkerClick(
		...
		marker
			.bindPopup(buildTrainPopupHTML(train), {
				maxWidth: 520,
				minWidth: 380,
				autoPan: false,
			})
		...
		try {
			...
			if (popup?.isOpen()) {
				...
				requestAnimationFrame(() => {
					...
					if (wrap && current) {
						const rowTop =
							(current as HTMLElement).offsetTop -
							(wrap as HTMLElement).offsetTop;
			...
			if (popup?.isOpen()) {
				popup.setContent(buildTrainPopupErrorHTML(e.train));
			}
		}
	}
	...
	useEffect(() => {
		...
		for (const train of trains) {
			...
			if (existing) {
				...
				if (rl && rlm !== null && train.status === "R") {
					...
					if (!projection.offRoute) {
						...
						if (existing.distanceAtPing === null) {
							...
							const advanced =
								existing.distanceAtPing + existing.pathSpeedMps * dtSec;
							const capped = Math.min(
								advanced,
								projection.targetDistanceAlongRoute,
							);
							existing.distanceAtPing = Math.max(
								0,
								Math.min(capped, existing.routeLengthMeters!),
							);
						}
						existing.targetDistanceAlongRoute =
							projection.targetDistanceAlongRoute;
				...
				marker.on("move", (e) =>
					hitMarker.setLatLng(
						(e as L.LeafletEvent & { latlng: L.LatLng }).latlng,
					),
				);
	...
	const focusTrain = useCallback(
		async (code: string, boardingStationCode?: string) => {
			...
			if (boardingStationCode) {
				...
				if (requestId !== focusRequestIdRef.current || !leafletMap.current)
					return "cancelled";
