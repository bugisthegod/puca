...
export default function OnboardingTour({ steps, onClose }: Props) {
  ...
  if (rect) {
    ...
    const clampedLeft = Math.max(
      EDGE_MARGIN,
      Math.min(rectCx - TOOLTIP_WIDTH / 2, viewport.w - TOOLTIP_WIDTH - EDGE_MARGIN),
    );
    tooltipStyle = placeBelow
      ? { top: rect.bottom + HIGHLIGHT_PADDING + TOOLTIP_GAP, left: clampedLeft }
      : { bottom: viewport.h - rect.top + HIGHLIGHT_PADDING + TOOLTIP_GAP, left: clampedLeft };
  } else {
    centered = true;
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={tooltipStyle}
      >
        <div className="tour__progress">{index + 1} / {steps.length}</div>
