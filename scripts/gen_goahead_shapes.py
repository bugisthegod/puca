...
Strategy (mirrors gen_buseireann_shapes.py):
  ...
  - Simplify polylines with RDP (20m tolerance, same as Dublin Bus / Bus Éireann).
...
def main():
    ...
    print(f"Go-Ahead trips: {len(all_trip_ids):,}", file=sys.stderr)
    ...
    missing_shapes = sum(1 for v in ga_shapes.values() for d in v.values() if not d["coords"])
    if missing_shapes:
        print(f"WARNING: {missing_shapes} route+directions have no shape coords", file=sys.stderr)
    no_stops = sum(1 for v in ga_shapes.values() for d in v.values() if not d["stops"])
    if no_stops:
