#!/usr/bin/env python3
"""
Generate src/data/goahead-schedule.db — SQLite schedule for Go-Ahead Ireland.

Schema (identical to bus-schedule.db and buseireann-schedule.db):
...
OUT_DB = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "../src/data/goahead-schedule.db")
)
...
def main():
    ...
    ga_route_ids: set[str] = set()
    with open(f"{GTFS_DIR}/routes.txt", newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row["agency_id"] in AGENCY_IDS:
                ga_route_ids.add(row["route_id"])
    print(f"  Go-Ahead route_ids: {len(ga_route_ids):,}")

    print("Reading trips.txt …")
    ga_trips: set[str] = set()
    with open(TRIPS_FILE, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row["route_id"] in ga_route_ids:
                ga_trips.add(row["trip_id"])
    print(f"  Go-Ahead trip_ids: {len(ga_trips):,}")
    ...
    with open(STOP_TIMES_FILE, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row["trip_id"] not in ga_trips:
                continue
            try:
